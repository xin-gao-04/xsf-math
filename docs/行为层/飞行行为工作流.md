...
本文档描述当前 `xsf-behavior` 的推荐工作流。

现在行为层分成两类：

- 原生制导程序层：更接近 `xsf-core`
- 原子控制器层：更接近外部框架托管动作
...
如果后续需要，可在保持轻量的前提下继续增加一层很薄的动作切换辅助结构，但不建议让仓库长成完整仿真内核。
