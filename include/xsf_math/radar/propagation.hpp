...
inline double two_ray_factor(double range_m, double h_tx_m, double h_rx_m, double frequency_hz) {
    ...
    double factor = 2.0 * sin_val;  // 电压因子
    return factor * factor;  // 功率因子（F^2）
}

// Blake 大气衰减模型（dB/km，随频率和俯仰角变化）
struct blake_attenuation {
    // 简化模型：衰减率随频率升高而增大
    // 随俯仰角增大而减小（高仰角穿越的大气更少）
    static double rate_db_per_km(double frequency_hz, double elevation_rad) {
        ...
        if (std::abs(elevation_rad) > 0.01) {
            ...
            el_factor = std::min(el_factor, 50.0);  // 在地平线处封顶
        }
...
struct rain_attenuation {
    double rain_rate_mm_per_hr = 0.0;  // 0 = 晴空
    ...
    double loss_db(double range_m, double frequency_hz) const {
        ...
        // 简化的 ITU 模型：gamma = k * R^alpha（dB/km）
        double k, alpha;
        if (f_ghz < 2.0) {
            k = 0.0001; alpha = 1.0;
        } else if (f_ghz < 10.0) {
...
struct multipath_model {
    double surface_reflection_coeff = -0.8;  // 海面典型值
    double surface_roughness_m      = 0.0;   // 均方根粗糙度
