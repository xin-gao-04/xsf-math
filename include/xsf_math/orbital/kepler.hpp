...
// 开普勒轨道力学（Keplerian orbital mechanics）

// 地球引力参数（Earth gravitational parameter, mu），单位 m^3/s^2
constexpr double mu_earth = 3.986004418e14;  // m^3/s^2

// 经典轨道根数（Classical orbital elements, COE）
struct orbital_elements {
    double semi_major_axis_m;  // 半长轴（Semi-major axis），单位 m
    double eccentricity;       // 偏心率（Eccentricity）：0=圆轨道，0<e<1=椭圆，1=抛物线，>1=双曲线
    double inclination_rad;    // 轨道倾角（Inclination），单位 rad
    double raan_rad;           // 升交点赤经（Right Ascension of Ascending Node, RAAN），单位 rad
    double arg_periapsis_rad;  // 近地点幅角（Argument of periapsis），单位 rad
    double true_anomaly_rad;   // 真近点角（True anomaly），单位 rad

    // 轨道周期（Orbital period），单位 s
    double period() const {
    ...
    // 相对地表的高度（Altitude above surface），单位 m
    ...
    // 当前轨道位置的速度（Orbital velocity at current radius），单位 m/s
    // 使用 vis-viva 方程：v^2 = mu * (2/r - 1/a)
    double velocity(double radius_m) const {
        return std::sqrt(mu_earth * (2.0/radius_m - 1.0/semi_major_axis_m));
    }

    // 当前真近点角对应的半径（Radius at current true anomaly），单位 m
    double radius() const {
        double p = semi_major_axis_m * (1.0 - eccentricity * eccentricity);  // 半通径（Semi-latus rectum），单位 m
        return p / (1.0 + eccentricity * std::cos(true_anomaly_rad));
    }
};

// 求解开普勒方程（Solve Kepler's equation）：M = E - e*sin(E)
// 已知平近点角 M 和偏心率 e，求偏近点角 E（Solve for eccentric anomaly E given mean anomaly M and eccentricity e）
inline double solve_kepler(double M_rad, double eccentricity, int max_iter = 30) {
    ...
    // 牛顿-拉夫森迭代（Newton-Raphson iteration）
...
// 将偏近点角转换为真近点角（Convert eccentric anomaly to true anomaly）
...
// 将真近点角转换为偏近点角（Convert true anomaly to eccentric anomaly）
...
// 由偏近点角求平近点角（Convert eccentric anomaly to mean anomaly）
inline double eccentric_to_mean_anomaly(double E_rad, double eccentricity) {
    return E_rad - eccentricity * std::sin(E_rad);
}

// 按时间增量推进轨道（二体开普勒模型）（Propagate orbit by time increment using two-body Kepler model）
inline orbital_elements propagate_kepler(const orbital_elements& oe, double dt_s) {
    double n = oe.mean_motion();  // 平均角速度（rad/s）

    // 当前偏近点角和平近点角（Current eccentric and mean anomaly）
...
// 将轨道根数转换为 ECI（Earth-Centered Inertial）位置和速度（Convert orbital elements to ECI position and velocity）
inline void elements_to_state(const orbital_elements& oe, vec3& pos, vec3& vel) {
    ...
    double p = a * (1.0 - e*e);  // 半通径（Semi-latus rectum），单位 m
    double r = p / (1.0 + e * std::cos(nu));

    // 轨道平面中的位置（Position in orbital plane）
    double x_orb = r * std::cos(nu);
    double y_orb = r * std::sin(nu);

    // 轨道平面中的速度（Velocity in orbital plane）
    double h = std::sqrt(mu_earth * p);  // 比角动量（Specific angular momentum），单位 m^2/s
    double vx_orb = -(mu_earth / h) * std::sin(nu);
    double vy_orb =  (mu_earth / h) * (e + std::cos(nu));

    // 从轨道平面旋转到 ECI（Rotate from orbital plane to ECI frame）
    ...
    // 旋转矩阵元素（Rotation matrix elements, R = R3(-RAAN) * R1(-i) * R3(-arg_periapsis)）
...
// 两颗卫星之间的视线可见性（Line-of-sight visibility between two satellites）
// 如果连线不与地球相交则返回 true（Returns true if the line segment does not intersect the Earth）
inline bool los_visible(const vec3& pos_a, const vec3& pos_b) {
    ...
    if (discriminant < 0.0) return true;  // 无交点（No intersection）

    double sqrt_disc = std::sqrt(discriminant);
    double t1 = (-b - sqrt_disc) / (2.0*a);
    double t2 = (-b + sqrt_disc) / (2.0*a);

    // 在区间 [0,1] 内有交点则表示被遮挡（Intersection within [0,1] means occulted）
    return !(t1 >= 0.0 && t1 <= 1.0) && !(t2 >= 0.0 && t2 <= 1.0);
}

// 大气阻力减速度（简化模型）（Atmospheric drag deceleration, simplified model）
// altitude_m: 海拔高度（m）; speed_mps: 速度（m/s）; drag_coeff_cd: 阻力系数; area_m2: 迎风面积（m^2）; mass_kg: 质量（kg）
inline double atmospheric_drag_accel(double altitude_m, double speed_mps,
                                      double drag_coeff_cd, double area_m2,
                                      double mass_kg) {
    // 用于轨道高度的指数大气密度模型（Exponential atmospheric density model for orbital altitudes）
    constexpr double rho0 = 1.225;    // 海平面大气密度（Sea-level density），单位 kg/m^3
    constexpr double H = 8500.0;      // 标度高度（Scale height），单位 m
